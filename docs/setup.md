...
- `ssh`:
    ...
    - `keypairs`: a map of named keypair objects — an arbitrary `{ name: { public, private } }` map (the name `*` remains a valid key name, alongside others also supported), each entry having `public` and `private` properties. **All** complete pairs are loaded into the devtainer's ssh-agent at launch; an incomplete or malformed entry is skipped, with a warning surfaced in the devtainer terminal. Add/remove via the CLI with `--set ssh.keypairs.<name>.public=...` / `--set ssh.keypairs.<name>.private=@<file>` and `--unset ssh.keypairs.<name>` (e.g. `{"*": {"public": "ssh-ed25519 AAAA... me@team", "private": "-----BEGIN OPENSSH PRIVATE KEY-----\n...\n-----END OPENSSH PRIVATE KEY-----\n"}, "github": {"public": "...", "private": "..."}}`)
